     function myFunction() {
           ...
           //function that calculates the cost for each hotel
           var calcCost = function ( checkIn , checkOut , Checkin_Day , Checkout_Day ) {
              var totalcost = 0;
              
              return  function (  seasonObject   ) {  //calculates the cost for each season

                var startDate = seasonObject.start.split("-");
                var endDate = seasonObject.end.split("-");
                ...
                var rate = seasonObject.rate.split("$");
           ...
           for (var i = 0; i < array.length; i++) {
                ...
                if (!array[i].hasOwnProperty("seasons")) {
                    ...
                    rental.totalcost = cost;
                        ...
                        var calc = calcCost( checkIn , checkOut , Checkin_Day , Checkout_Day );
                        for (var j = 0; j < array[i].seasons.length; j++) {
                            for (var key in array[i].seasons[j]) {
                                // "calc" calculates the cost for each season
                                rental.totalcost = calc( array[i].seasons[j][key]  );
                            }
                        }
                        var clean = 0;
                        if ( array[i].hasOwnProperty("cleaning fee")){
